                    ...
                    <div class="card-body pb-0">
                        <div class="d-flex justify-content-between">
                            ...
                            <form action="" method="get">
                                <div class="d-flex">
                                    <input value="{{ Request::get('keyword') }}" type="text" class="form-control" name="keyword" id="keyword" placeholder="Search">
                                    ...
                                    @foreach($books as $book)
                                        @php
                                            if($book->reviews_count > 0)
                                                ...
                                                $avgRating = 0;
                                            $avgRatingPr = ($avgRating*100)/5;
                                        @endphp
                                        <tr>
                                            <td>{{ $book->title }}</td>
                                            <td>{{ $book->author }}</td>
                                            <td>{{ number_format($avgRating,1) }} ({{ ($book->reviews->count() > 1) ? $book->reviews->count().' Reviews' :  $book->reviews->count().' Review' }})</td>
                                            <td>
                                                @if($book->status == 1)
